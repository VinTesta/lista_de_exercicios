...
função verificarIdade(idade)
    ...
    senão se idade >= 16 e idade < 18 então
        imprimir("Não é obrigado a votar")
    senão
        imprimir("É obrigado a votar")
fim função
```
______
...
#### R:
```
função corrida(aceleracao, distancia, velocidade, max, tempoMaximo):
  tempo = 0
  ...
  enquanto verdadeiro:
    ...
    se velocidade > max então
      velocidade = max

    distanciaPercorrida = velocidade * tempo + 0.5 * aceleracao * tempo^2
    ...
    se tempo >= tempoMaximo então
      imprimir("O tempo máximo de corrida foi atingido. O carro não completou a corrida.")
      parar
    ...
    Senão:
        ...
        Para i de 0 até linhasA-1 faça:
            Para j de 0 até colunasB-1 faça:
                ...
                # Loop para multiplicação dos elementos e soma dos produtos
